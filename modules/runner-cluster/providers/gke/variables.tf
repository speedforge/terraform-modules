...
variable "kubeconfig" {
  description = "Reference to the GKE kubeconfig. Contains references only, not secret values."
  type = object({
    source  = string
...
variable "node_pools" {
  description = "GKE node pools normalized to the runner-cluster contract."
  type = map(object({
