...
variable "kubeconfig" {
  description = "Reference to the Rackspace Spot kubeconfig. Contains references only, not secret values."
  type = object({
    source  = string
...
variable "node_pools" {
  description = "Rackspace Spot node pools normalized to the runner-cluster contract."
  type = map(object({
